...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let letterQuery = await sb
      .from("letters")
      .select("id, church_totvs_id, preacher_user_id, status, storage_path, url_pronta, url_carta, pdf_url")
      .eq("id", letter_id)
      .maybeSingle();
    ...
    if (letterQuery.error && String(letterQuery.error.message || "").toLowerCase().includes("url_carta")) {
      letterQuery = await sb
        .from("letters")
        .select("id, church_totvs_id, preacher_user_id, status, storage_path, url_pronta, pdf_url")
        .eq("id", letter_id)
        .maybeSingle();
    }
    ...
    const pdfUrl = String((letter as Record<string, unknown>).pdf_url || "").trim();
    if (pdfUrl.startsWith("http://") || pdfUrl.startsWith("https://")) {
      return json({ ok: true, url: pdfUrl, source: "pdf_url" }, 200);
    }
