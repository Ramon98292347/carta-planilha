...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Comentario: se o usuario ja existe, mantemos o role atual na edicao.
    ...
    // Comentario: pastor so consegue cadastrar usuario dentro do proprio escopo.
    ...
    const payload: Record<string, unknown> = {
      ...
      can_create_released_letter: typeof body.can_create_released_letter === "boolean" ? body.can_create_released_letter : false,
    };
    ...
    const { data: saved, error: saveErr } = await sb
      ...
      .select("id, cpf, full_name, role, default_totvs_id, totvs_access, is_active, can_create_released_letter, updated_at")
      .single();
