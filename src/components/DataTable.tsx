...
const getDerivedStatusLabel = (row: Record<string, string>) => {
  ...
  const liberacaoAutomatica = String(
    row.liberacaoAutomatica || row.liberacao_automatica || row["Libera��o Autom�tica"] || row["Liberacao Automatica"] || "false"
  )
    .trim()
    .toLowerCase();
...
export function DataTable({
  ...
  const isLiberacaoAutomatica = (row: Record<string, string>) => {
    const raw = String(row.liberacaoAutomatica || row.liberacao_automatica || row["Libera��o Autom�tica"] || row["Liberacao Automatica"] || "false")
      .trim()
      .toLowerCase();
